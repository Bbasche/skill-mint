...
class DB {
     ...
     */
    constructor(kwil, payloadType, options) {
        this.description = '';
        this.kwil = kwil;
     ...
     */
    static create(kwil, payloadType, options) {
        objects_1.objects.requireNonNil(kwil, 'client is required for DbBuilder. Please pass a valid Kwil client. This is an internal error, please create an issue.'),
            objects_1.objects.requireNonNil(payloadType, 'payloadType is required for DbBuilder. Please pass a valid PayloadType. This is an internal error, please create an issue.');
        return new DB(kwil, payloadType, options);
    }
    /**
    ...
    async buildTx() {
        ...
        const ensuredPayload = objects_1.objects.requireNonNil(this.payload, 'dbBuilder payload cannot be null');
        ...
        let cleanedPayload = () => ensuredPayload();
        if (this.payloadType === enums_1.PayloadType.DEPLOY_DATABASE) {
            // make the payload encodable
            const encodablePayload = this.makePayloadEncodable(ensuredPayload);
            ...
            db.tables.forEach((table) => {
                ...
                table.columns &&
                    table.columns.forEach((column) => {
                        ...
                        if (!column.type.name) {
                            column.type.name = enums_1.VarType.NULL;
                        }
                ...
                procedure.parameters &&
                    procedure.parameters.forEach((parameter) => {
                        ...
                        if (!parameter.type.name) {
                            parameter.type.name = enums_1.VarType.NULL;
                        }
                ...
                if (!Array.isArray(procedure.return_types)) {
                    ...
                    procedure.return_types.fields &&
                        procedure.return_types.fields.forEach((field) => {
                            ...
                            if (!field.type.name) {
                                field.type.name = enums_1.VarType.TEXT;
                            }
            ...
            db.foreign_calls.forEach((foreignCall) => {
                ...
                foreignCall.parameters &&
                    foreignCall.parameters.forEach((parameter) => {
                        if (!parameter.name) {
                            parameter.name = enums_1.VarType.NULL;
                        }
    ...
    validateCreateDBOptions() {
        const client = (0, validation_1.validateField)(this.kwil, 'client');
        const payloadType = (0, validation_1.validateField)(this.payloadType, 'payload type');
        return { client, payloadType };
    }
    validateDBTxOptions() {
        const payloadType = (0, validation_1.validateField)(this.payloadType, 'payload type');
        const signer = (0, validation_1.validateField)(this.signer, 'signer');
        const identifier = (0, validation_1.validateField)(this.identifier, 'identifier');
        const chainId = (0, validation_1.validateField)(this.chainId, 'chainId');
        const signatureType = (0, validation_1.validateField)(this.signatureType, 'signature type');
